...
import {
    checkUsernameWeb3,
    connectToContract,
    deligateDocument, getDelegateePublicKey,
    getDeligatedDocumentIds,
    getDeligatedDocumentInfo,
    getUploadedDocumentIds,
    getUploadedDocumentInfo, signInUser,
    uploadADocument
} from "./web3Interactions";
import {connectNode, getData, handleUpload} from "./ipfsInteractions";

async function doConnections(_contractAddress){
    await connectToContract(_contractAddress);
    await connectNode();
}

async function checkUsernameAvailability(_username){
    return await checkUsernameWeb3(_username);
}

async function signUpAndGetNucypherKeys(_username,
                                        _aliceEthereumPrivateKey,
                                        _callingObject) {
    let rv = await generateKeyPairs();
    await signInUser(_username, _aliceEthereumPrivateKey, rv.alicePublicKey, _callingObject);
    return rv;
}

async function uploadDocument(_array,
                              _privateKey,
                              _uploader,
                              _alicePublicKey,
                              _aliceEthereumPrivateKey,
    ...
    await uploadADocument(cipherText,
        capsule,
        _aliceVerifyKey,
        _alicePublicKey,
        _uploader,
...
async function grantDocumentAccess(_documentId,
                                   _alicePrivateKey,
                                   _aliceSigningKey,
                                   _deligatee,
                                   _uploader,
    ...
    let policyId = await createPolicy(_alicePrivateKey, _aliceSigningKey, bobPublicKey);

    // Update the contract regarding delegation
    await deligateDocument(_documentId, policyId, _deligatee, _uploader, _aliceEthereumPrivateKey, _callingObject);
}


async function fetchUploadedDocuments(_uploader,
                                      _aliceEthereumPrivateKey,
                                      _alicePrivateKey,
                                      _requestedObject,
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        let { cipherText, capsule, verifyKey, publicKey } = await getUploadedDocumentInfo(arr[i], _aliceEthereumPrivateKey);

        // Use this info to generate the ipfs hash
        let ipfsHash = await decryptUploadedDocument(cipherText, capsule, _alicePrivateKey);
...
async function fetchDelegatedDouments(_deligatee,
                                      _aliceEthereumPrivateKey,
                                      _alicePublicKey,
                                      _alicePrivateKey,
                                      _requestedObject,
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        let { cipherText, capsule, verifyKey, publicKey, policyId } = await getDeligatedDocumentInfo(arr[i], _deligatee, _aliceEthereumPrivateKey);

        // Use this info to generate the ipfs hash
        let ipfsHash = await decryptDeligatedDocument(publicKey, verifyKey, _alicePublicKey, _alicePrivateKey, capsule, cipherText, policyId);

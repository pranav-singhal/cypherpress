 ...
 */
async function signInUser(_username, _privateKey, _alicePublicKey, _callingObject) {
    return new Promise(function (resolve, reject) {
 ...
 */
async function uploadADocument(_cipherText,
                               _capsule,
                               _aliceVerifyKey,
                               _alicePublicKey,
                               _uploader,
    ...
    return new Promise(function (resolve, reject) {
        let address = web3.eth.accounts.privateKeyToAccount(_privateKey).address;
        contract.methods
            .uploadADocument(_cipherText, _capsule, _aliceVerifyKey, _alicePublicKey, _uploader)
            .estimateGas({
                from: address
            })
            .then(async function(gasAmount) {
                web3.eth.getGasPrice()
                    .then(function (gasPrice) {
                        ...
                        web3.eth.getBalance(address).then(bal => {
                            ...
                            console.log(typeof requiredEth);
                            if (bal > requiredEth) {
                                _callingObject.verifyTransaction(transaction, gasInEth, "Upload Document to Contract", function() {
                                    let signPromise = web3.eth.accounts.signTransaction(
                                        transaction,
                                        _privateKey
                                    );
 ...
 */
async function deligateDocument(_documentId,
                                _policyId,
                                _deligatee,
                                _uploader,
                                _privateKey,
